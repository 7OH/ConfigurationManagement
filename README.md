...
![Версия](https://img.shields.io/badge/Версия-0.3.9.2-1F6FEB) ![.NET](https://img.shields.io/badge/.NET-10-512BD4) ![Windows/WPF](https://img.shields.io/badge/Windows-WPF-4B8BBE) ![Linux/Avalonia](https://img.shields.io/badge/Linux-Avalonia%2011-8B5CF6) ![Лицензия](https://img.shields.io/badge/Лицензия-Open%20Source-success)
...
- **Проверка обновлений конфигураций 1С (F9)** — проверка для выбранной ИБ по web-ресурсу обновлений 1С, подсветка нового релиза и загрузка дистрибутива; поддерживается авторизация на сайте 1С (логин/пароль, HTTP Basic Auth) в Настройках для доступа к защищённым каталогам `downloads.1c.ru`.
...
- **Действие по двойному щелчку на базе** — «Запустить 1С:Предприятие», «Запустить Конфигуратор» или «Ничего»; глобально (Настройки → Базы) и индивидуально для каждой ИБ (в свойствах, переопределяет глобальное).
...
- **Режимы функциональности: Пользователь / Специалист / Разработчик** — ограничение доступа в системное меню для режима «Пользователь» (Настройки → Настройки → «Режим функциональности»): в режиме «Пользователь» в контекстном меню базы остаются только запуск, избранное и закрепление.
